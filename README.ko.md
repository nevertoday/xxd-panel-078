...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 078이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
정체성·윤곽·구조·자세·관계 추출 → 정교한 선형으로 요약 → 코튼 종이 위 하나의 시각 중심 → 샴페인 금박을 홈 안으로 깊게 압인 → 무잉크 압인을 보조 구조로 사용 → 종이 섬유·가장자리 빛·압력 그림자 → 소량의 글자도 같은 공정으로 압인
```

## 완성작의 특징

- 금박 선은 표면에서 솟지 않고 실제로 안쪽으로 눌린 홈 안에 놓입니다.
- 압축된 종이, 가장자리의 미세한 빛, 자연스러운 압력 그림자와 보이는 섬유를 유지합니다.
- 소량의 무잉크 블라인드 디보스를 더 조용한 두 번째 구조로 사용합니다.
- 하나의 시각 중심과 넓고 고요한 여백을 유지합니다.
- 가늘고 현대적인 편집 서체를 도형과 같은 공정으로 종이 안에 압인합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-078-prompt.en.md)
...
78개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 078까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–078) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
