...
# 🦁 XXD Panel 078｜금박 압인 저널

### 사진의 핵심 윤곽을 아이보리 코튼 종이의 샴페인 골드 오목 압인으로 바꿉니다
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 078 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 078 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 078 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 078 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 078 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 078 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 078 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 078 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 078이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
**Panel 078**는 피사체의 윤곽, 구조, 자세와 관계를 가는 선형 표장으로 요약하고 샴페인 금박과 무잉크 블라인드 디보스를 따뜻한 아이보리 코튼 종이에 깊이 눌러 넣습니다. 금선은 실제 홈 안에 있으며 압축 섬유, 가장자리 빛과 압력 그림자가 오목함을 증명합니다.

### 이런 경우에 적합합니다

- 건축, 인물, 동식물, 사물, 교통수단, 풍경을 알아볼 수 있는 정교한 선형 표장으로 바꾸고 싶을 때
- 피사체의 정체성, 윤곽, 구조, 자세와 관계를 보존하면서 사실적 세부는 절제하고 싶을 때
- 따뜻한 코튼 종이, 홈 속 샴페인 금박, 무잉크 블라인드 디보스가 만드는 조용한 고급감을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 078로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 고급 향수, 주얼리, 웨딩 문구 같은 샴페인 골드 오목 압인 작품 |
| **대표 특징** | 실제 홈 안 금선과 종이 압축, 가장자리 빛, 압력 그림자, 보이는 코튼 섬유 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-078`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-078-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-078-prompt.zh-CN.md)
...
**특징:** 아이보리 코튼 종이 · 딥 디보스 · 홈 안 샴페인 금박 · 가는 선형 표장 · 무잉크 압인 · 절제된 고급감
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 078의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-078 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-078 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-078 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-078 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-078 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-078 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 078 적용, 공통 설정은 한 번만 확정
/xxd-panel-078 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–078을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
