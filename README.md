...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 078 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

很多“烫金高级感”只是把亮黄色金属贴在表面，甚至做成塑料凸起；它有光泽，却没有纸张受压后的真实触感与安静分量。

**Panel 078** 把主体最具识别力的轮廓、结构、姿态和关系概括成纤细线性标识，再用香槟金箔与无墨 blind deboss 一起深压进暖米白棉质艺术纸。金线必须位于真实凹槽内部，纸纤维在边缘被挤压并形成细微高光和压力阴影；成品像高端香氛、珠宝与婚礼纸品，而不是表面贴金或凸起浮雕。
...
> 用 XXD Panel 078 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 暖米白棉质艺术纸 · 深压凹印 · 凹槽香槟金箔 · 纤细线性标识 · 无墨压痕 · 低调奢华

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 078 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像高端香氛、珠宝或婚礼纸品视觉系统的香槟金凹压作品 |
| **一眼可辨的特点** | 金箔细线位于真实凹槽内，伴随纸张挤压、边缘微光、压力阴影与可见棉纸纤维 |
| **它如何尊重你的输入** | 保留最关键的身份、轮廓、结构、姿态和关系，再简化为一眼可辨的纤细线性图形 |
...
```text
提取主体身份、轮廓、结构、姿态与关系 → 概括为纤细线性图形 → 在暖米白棉纸上建立唯一视觉核心 → 将香槟金线深压进真实凹槽 → 以无墨凹压补充第二层结构 → 用纸纤维、边缘微光与压力阴影证明凹陷 → 同工艺压入少量优雅文字
```

## 成品中最容易识别的风格特点

- 主体以纤细描线、简化轮廓和极少结构细节保持身份，不做复杂写实插画。
- 香槟金箔必须位于向内凹陷的压槽中，四周可见纸张挤压、边缘高光和自然压力阴影。
- 少量无墨 blind deboss 作为更低调的第二层结构，与金色凹压形成深浅关系。
- 构图只保留一个视觉核心和大量安静留白，不加入无来源装饰。
- 文字采用纤细、略带古典雕刻韵味的现代编辑字体，与图形一次压入纸面。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-078-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 078 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-078 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-078 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-078 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 078；公共设置只解析一次
/xxd-panel-078 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-078`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-078-prompt.en.md) · [中文运行适配器](references/xxd-panel-078-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

78 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–078 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–078） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
