...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 078 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Much “luxury foil” simply places bright yellow metal on the surface or turns it into plastic relief. It shines, but lacks the quiet weight and physical truth of compressed paper.

**Panel 078** reduces the source's defining contour, structure, pose, and relationship into fine emblematic linework, then deep-presses champagne foil and inkless blind deboss into warm ivory cotton paper. Gold must sit inside a real recess, with compressed fibres, subtle edge light, and pressure shadow proving the indentation. The result belongs to fine fragrance, jewellery, and wedding stationery—not raised foil or surface decoration.

### Best for

- understated luxury led by deboss depth, cotton fibre, quiet whitespace, and restrained champagne reflection;
- converting architecture, people, animals, plants, objects, or landscapes into recognisable fine-line identity marks;
- physically correct inward foil recesses rather than raised stamping, mirror gold, or plastic relief;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 078 on this image and recommend the most suitable composition and size first.
...
**Signature:** warm ivory cotton paper · deep deboss · recessed champagne foil · fine-line emblem · blind pressure · understated luxury
...
|---|---|
| **What you get** | A champagne-gold recessed print resembling a fine fragrance, jewellery, or wedding-stationery identity system |
| **Recognisable signature** | Foil lines inside real grooves with paper compression, edge highlights, pressure shadows, and visible cotton fibre |
| **How it respects the source** | It keeps defining identity, contour, structure, pose, and relationship, then simplifies them into recognisable fine linework |
...
```text
extract identity, contour, structure, pose, and relationship → reduce to fine linear graphics → establish one core on warm ivory cotton paper → deep-press champagne lines into real recesses → add inkless blind deboss as a second structure → prove indentation through fibre compression, edge light, and pressure shadow → press minimal elegant typography in the same operation
```

## Recognisable finished traits

- Fine linework, simplified contour, and minimal structural detail preserve identity without complex realistic illustration.
- Champagne foil sits inside inward grooves surrounded by compressed paper, subtle edge light, and natural pressure shadow.
- Inkless blind deboss may form a quieter second layer beside the recessed gold.
- One visual core and abundant quiet whitespace replace unsupported decoration.
- Fine modern editorial type with a hint of classical engraving is pressed into the sheet with the image.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-078-prompt.en.md)
...
```text
/xxd-panel-078 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-078 photo.jpg --prefs last
/xxd-panel-078 photo.jpg --prefs edit
/xxd-panel-078 photo.jpg --prefs new
```
...
/xxd-panel-078 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-078 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-078 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-078 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-078 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-078 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-078 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-078 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-078 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-078 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-078 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 078 to a whole directory; shared settings resolve once
/xxd-panel-078 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-078 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-078.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-078" ~/.codex/skills/xxd-panel-078
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-078 --skill xxd-panel-078
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-078`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-078-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-078-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 078; the current project is bold.
